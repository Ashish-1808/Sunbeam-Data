...
Dynamic Web page (having typically  HTML 5 markup) , can contain Java code directly.
...
Java Beans (Service layer)or JSP Custom Tags(actions) --- will contain Business logic.
...
JSP API : is a part of Java EE (Jakarta EE)  specs
jsp-api.jar --- <tomcat>/lib : specs

Contains Tomcat's JSP API implementation classses. : jasper.jar


0. jakarta.servlet.Servlet -- super i/f
1. jakarta.servlet.jsp.JspPage -- extends Servlet i/f
1.1 public void jspInit()
1.2 public void jspDestroy()

Can be overridden by JSP page author

2. Further extended by  jakarta.servlet.jsp.HttpJspPage
...
7. The 1st it calls : public void jspInit() : one time inits can be performed.(jspInit availble from jakarta.servlet.jsp.JspPage)
8. Then it will call follwing method using thread created per clnt request : 
public void _jspService(HttpServlet Rq,HttpServletResponse) throws ServletException,IOException(API avlble from jakarta.servlet.jsp.HttpJspPage)
...
2.1 out - jakarta.servlet.jsp.JspWriter : represents the buffered writer stream connected to the clnt via HttpServletResponse(similar to your PrintWriter in servlets)
...
2.4 session : HttpSession (By default all JSPs participate in session tracking i.e session obj is created) 

2.5 exception : java.lang.Throwable (available only to err handling pages)

2.6 pageContext  : current page environment : jakarta.servlet.jsp.PageContext(this class stores references to page specific objects viz -- exception,out,config,session.....)
Usage
...
More concise alternative to JSP Expressions(typically for accessing attributes) : EL syntax (Expression Language : avlble from JSP 1.2 onwards)
...
${param.email}
Java code - param.get("email")
OR
request.getParamter("email")

pageScope=Name of the map ,  created by WC :  containing   page scoped attributes
eg : ${pageScope.attrName}

requestScope=map of request scoped attributes
sessionScope=map of session scoped attributes
applicationScope=map of application(=ServletContext) scoped attributes
...
3.1 errorPage="URI|path  of error handling page" 
To tell WC to forward user to error handler page , in case of errors.
3.2 isErrorPage="true|false" default = false
If you enable this to true--- WC will supply   'exception' & errorData objects to this page

Instead of adding errorPage attribute , in multiple JSPs ,
add xml tag , in web.xml

This exception obj is stored under current page ---i.e under pageContext (type=jakarta.servlet.jsp.PageContext -- class which represents curnt JSP)
...
Servlet class can imple. tag i/f(marker i/f) -- jakarta.servlet.SingleThreadModel(DEPRECATED) -Marker interface
...
--- has become standard part of Java EE  specifications from version 1.5 onwards.
---JSTL implementation  support exists in 2 separate  JARs -
jakarta.servlet.jsp.jstl-3.0.1.jar
jakarta.servlet.jsp.jstl-api-3.0.2.jar

JSTL Dev steps
1.Copy above JARs into your run-time classpath/build path(copy jars either in <tomcat_home>/lib OR <WEB-INF>/lib
- not already done!

2. Use taglib directive to import JSTL supplied  tag library into  JSP.
tag=action
...
JAR 
- consists of Tag implementation classes
Tag library-   TLD -- Tag library descriptor -- desc of tags -- how to use tags
