...
public class AppointmentServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		try {
			// 1. Get request param - appointment id
			String appId = request.getParameter("id");
			// 2. null checking
			if (appId != null) {
				// cancel appointment -  parse appointment id 
			...
			{
				//show book appointment form (i.e forward the client to JSP)
				RequestDispatcher rd=request.getRequestDispatcher("/WEB-INF/views/book_appointment.jsp");
